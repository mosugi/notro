...
このリポジトリは **npm workspace モノレポ** で、4 つのパッケージを含んでいます：
...
| [`notro`](./packages/notro/) | `packages/notro/` | Astro + Notion API 統合ライブラリ。Content Loader・MDX コンパイルパイプライン（内部で `remark-nfm` を使用）・ヘッドレス Astro コンポーネントを提供。 |
| [`notro-ui`](./packages/notro-ui/) | `packages/notro-ui/` | `notro` 向けのコピー所有型スタイル済みコンポーネント（shadcn/StarwindUI と同じ思想）。`npx notro-ui init` でプロジェクトにコンポーネントをインストール — インストール後はあなたのコードになり、直接編集できます。 |
| `notro-tail` | `template/` | デプロイ可能な Astro スターターテンプレート。リファレンス実装として使用され、`create-notro`（`npm create notro@latest`）でも取得されます。`npm create astro@latest -- --template mosugi/notro-tail/template` でも直接利用可能。 |
