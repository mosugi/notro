...
| `notro-tail` (blog) | `templates/blog/` | フル機能ブログテンプレート。リファレンス実装として使用され、`create-notro`（`npm create notro@latest`）でも取得できます。スタイル済み Notion ブロックコンポーネントを `src/components/notro/` に内包しています。 |
