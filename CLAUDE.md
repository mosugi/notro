...
- **既存コードの再利用を常に最初に検討すること**。新しいヘルパーや抽象化を作る前に、既存の関数・コンポーネントで対応できないか確認する
- **コードの総量を増やさないこと**。コード量の増加はメンテナンスコストに直結する。シンプルな解決策を優先し、過剰な共通化・抽象化は避ける
- **後方互換・既存実装の変更回避を過度に意識しないこと**。不要な後方互換シムや未使用コードの温存はコード量を増やすだけ。既存コードを直接変更・削除することを恐れない
- 削除できる未使用コードは積極的に削除すること
